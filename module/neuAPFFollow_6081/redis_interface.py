...
def read_follow_path(redis_conn, u2m=unit_to_meter): # 读取跟随路径 
    fp_lonlat_str = get_or_default(redis_conn, "Navi", "GPath", default='$GP,0')
    fp_list = fp_lonlat_str.split(',')
    fp_list = [x for x in fp_list if x is not None and x !='']
    head = fp_list[0]
    path = []
    if head == '$GP' and len(fp_list[2:])==int(fp_list[1])*2: 
        for i in range(2, len(fp_list), 2):
            # path.append(convert_lonlat_to_abs_xy((float(gp_list[i]), float(gp_list[i + 1])), u2m))
            path.append((float(fp_list[i]), float(fp_list[i + 1])))
    return path
